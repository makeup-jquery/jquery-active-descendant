...
function updateActiveDescendantItems(){$descendantItems=$widget.find(descendantItemsSelector);$descendantItems.nextId();$descendantItems.each(function(idx,itm){$(itm).data(pluginName,{'idx':idx});});if($descendantItemsContainer===undefined&&$descendantItems.length>0){$descendantItemsContainer=getDescendantItemsCommonAncestor();$descendantItemsContainer.nextId();$focusItem.attr('aria-owns',$descendantItemsContainer.prop('id'));}}
$widget.on('activeDescendantItemsChange',updateActiveDescendantItems);$focusItem.on('blur',function(){$focusItem.removeAttr('aria-activedescendant');$widget.find('[aria-selected=true]').removeAttr('aria-selected');});$focusItem.on('downArrowKeyDown',function onDownArrowKeyDown(e){if($descendantItems.length>0){var $activeDescendant=$widget.find('#'+$focusItem.attr('aria-activedescendant'));var $newActiveDescendant;if($activeDescendant.length===0){$newActiveDescendant=$descendantItems.first();}else{var itemIdx=$activeDescendant.data(pluginName).idx;var $nextEl=$descendantItems.eq(itemIdx+1);var hasNextEl=$nextEl.length===1;var $firstEl=$descendantItems.eq(0);$newActiveDescendant=(hasNextEl)?$nextEl:$firstEl;}
updateDom($activeDescendant,$newActiveDescendant);}});$focusItem.on('upArrowKeyDown',function onUpArrowKeyDown(e){if($descendantItems.length>0){var $activeDescendant=$widget.find('#'+$focusItem.attr('aria-activedescendant'));var $newActiveDescendant;if($activeDescendant.length===0){$newActiveDescendant=$descendantItems.last();}else{var itemIdx=$activeDescendant.data(pluginName).idx;var $prevEl=$descendantItems.eq(itemIdx-1);var hasPrevEl=$prevEl.length===1;var $lastEl=$descendantItems.eq($descendantItems.length-1);$newActiveDescendant=(hasPrevEl)?$prevEl:$lastEl;}
updateDom($activeDescendant,$newActiveDescendant);}});updateActiveDescendantItems();});};}(jQuery,window,document));