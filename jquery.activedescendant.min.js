...
(function($,window,document,undefined){var pluginName='jquery-active-descendant';$.fn.activeDescendant=function activeDescendant(focusItem,descendantItems){return this.each(function onEach(){var $widget=$(this);var $focusItem=$widget.find(focusItem);var $descendantItems;$focusItem.commonKeyDown();function updateDom($activeDescendant,$newActiveDescendant){$focusItem.attr('aria-activedescendant',$newActiveDescendant.prop('id'));$activeDescendant.removeClass('activedescendant');$newActiveDescendant.addClass('activedescendant');$widget.trigger('activeDescendantChange',$newActiveDescendant);}
$widget.on('updateActiveDescendantCollection',function(e,collection){$descendantItems=$widget.find(collection);$descendantItems.nextId();$descendantItems.each(function(idx,itm){$(itm).data(pluginName,{'idx':idx});});});$focusItem.on('blur',function(){$('#'+$focusItem.attr('aria-activedescendant')).removeClass('activedescendant');$focusItem.removeAttr('aria-activedescendant');});$focusItem.on('downArrowKeyDown',function onDownArrowKeyDown(e){if($descendantItems.size()>0){var $activeDescendant=$widget.find('#'+$focusItem.attr('aria-activedescendant'));var $newActiveDescendant;if($activeDescendant.size()===0){$newActiveDescendant=$descendantItems.first();}else{var itemIdx=$activeDescendant.data(pluginName).idx;var $nextEl=$descendantItems.eq(itemIdx+1);var hasNextEl=$nextEl.length===1;var $firstEl=$descendantItems.eq(0);$newActiveDescendant=(hasNextEl)?$nextEl:$firstEl;}
